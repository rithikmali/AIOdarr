...
def processor(mock_config):
    ...
    with patch("src.processor.RadarrClient"), patch("src.processor.AIOStreamsClient"):
...
def test_process_movie_no_imdb_id(processor):
    ...
    movie = {"id": 1, "title": "Test Movie", "year": 2024, "imdbId": None}
...
def test_process_movie_no_streams_found(processor):
    ...
    movie = {"id": 1, "title": "Test Movie", "year": 2024, "imdbId": "tt1234567"}
...
def test_process_movie_success_with_url(processor):
    ...
    processor.aiostreams.search_movie = Mock(
        return_value=[
            {
...
def test_trigger_aiostreams_download(mock_head, processor):
    ...
    mock_head.assert_called_once_with(
        "https://example.com/stream", timeout=30, allow_redirects=True
    )


def test_process_wanted_movies(processor):
    """Test processing all wanted movies"""
    processor.radarr.get_wanted_movies = Mock(
        return_value=[
